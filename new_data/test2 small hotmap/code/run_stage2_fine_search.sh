...
if [[ -z "${DATA_DIR:-}" ]]; then
  ...
  if [[ -f "$SERVER_DATA_DIR/data_build_config.json" ]]; then
    ...
    echo "processed_data was not found beside test2 small hotmap or in its parent archive" >&2
    exit 2
  fi
fi
...
if ! [[ "$WORKERS" =~ ^[1-9][0-9]*$ ]] || (( WORKERS > 24 )); then
  echo "WORKERS must be an integer from 1 to 24" >&2
  exit 2
fi
...
LOCK_DIR="$RESULTS_DIR/.stage2_fine_search.lock"
...
echo "PRIMARY_RANKING=pooled RMSE over frozen H1-H4 cooling windows"
...
echo "PHASE 2/3: validation-only hard-cooling summary and one-SE candidates"
...
echo "HARD_MEAN_MATRIX=$RESULTS_DIR/validation_summary/hard_cooling_rmse_mean_matrix.csv"
echo "FULL_MEAN_MATRIX=$RESULTS_DIR/validation_summary/full_validation_rmse_mean_matrix.csv"
echo "RECOMMENDATION=$RESULTS_DIR/validation_summary/fine_search_recommendation.json"
