...
class ParallelGRUGraph(nn.Module):
    def __init__(
        self,
        gate_mode: str,
        feature_names: Sequence[str],
        ...
        super().__init__()
        self.history = nn.GRU(len(feature_names), config.history_hidden, batch_first=True)
        self.graph = PhysicalGraphEncoder(
            feature_names, history_mean, history_scale, config.graph_hidden,
            config.graph_sweeps, gate_mode, config.edge_hidden, config.kan_grid,
            config.dropout,
...
def build_model(
    ...
    if config.model_name == "parallel_gru_mlp_gnn":
        return ParallelGRUGraph("mlp", feature_names, history_mean, history_scale, len(control_names), control_horizon, config)
    if config.model_name == "parallel_gru_kan_gnn":
        return ParallelGRUGraph("kan", feature_names, history_mean, history_scale, len(control_names), control_horizon, config)
