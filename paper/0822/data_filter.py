#!/usr/bin/env python3
"""Causal sensor filtering and filter-quality review for the raw 0822 dataset.
...
- time columns and all valve columns are kept unchanged;
...
DEFAULT_INPUT_DIR = SCRIPT_DIR / "original_data"
...
# All control valves are protected exactly.  COOLDOWN also appears under the
# historical export typo ``Cooldowm`` and is normalized only for recognition;
# the source column name and values remain unchanged.
MAIN_VALVES = (
    "CV8300",
    "CV8313",
    "CV8310",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review or apply leakage-safe causal filters to raw, unaugmented 0822 CSV data. "
...
def canonical_signal_name(column: str) -> str:
    upper = signal_name(column).upper().replace(" ", "")
    return MODULE_VALVE_ALIASES.get(upper, upper)
...
def signal_kind(column: str) -> str:
    ...
    if str(column).strip().endswith(" Time") or "TIME" in upper:
        return "time"
    if upper.startswith("CV") or upper in MODULE_VALVE_ALIASES:
        return "valve"
...
def select_valve_events(
    ...
    valve_columns = [
        column
        for column in frame.columns
        if signal_kind(column) == "valve" and canonical_signal_name(column) in MAIN_VALVES
    ]
...
def find_value_column(frame: pd.DataFrame, requested_signal: str) -> str | None:
    for column in frame.columns:
        if (
            canonical_signal_name(column) == canonical_signal_name(requested_signal)
            and column.endswith(" ValueY")
        ):
            return column
    return None
...
def run_review(args: argparse.Namespace) -> None:
    ...
    config = {
        ...
        "valve_smoothing": False,
        "valve_negative_clamp_in_apply_mode": True,
        "valve_minimum": 0.0,
...
def clamp_negative_valves(frame: pd.DataFrame) -> Tuple[int, Dict[str, int]]:
    """Apply only the approved physical lower bound to valve measurements."""
    total = 0
    by_signal: Dict[str, int] = {}
    for column in frame.columns:
        if signal_kind(column) != "valve" or not str(column).endswith(" ValueY"):
            continue
        ...
        if count:
            ...
            total += count
            signal = canonical_signal_name(column)
            by_signal[signal] = by_signal.get(signal, 0) + count
    return total, by_signal


def valve_matches_lower_bound_contract(before: pd.Series, after: pd.Series) -> bool:
    expected = pd.to_numeric(before, errors="coerce").to_numpy(dtype=float)
    actual = pd.to_numeric(after, errors="coerce").to_numpy(dtype=float)
    expected = expected.copy()
    expected[expected < 0.0] = 0.0
    return bool(np.allclose(expected, actual, rtol=0.0, atol=1e-12, equal_nan=True))
...
def verify_protected_columns(before: pd.DataFrame, after: pd.DataFrame) -> Tuple[bool, List[str]]:
    ...
    for column in before.columns:
        ...
        if column not in after:
            ...
            if not valve_matches_lower_bound_contract(before[column], after[column]):
                changed.append(column)
        elif kind in {"time", "excluded_temperature", "other"}:
            if not series_semantically_equal(before[column], after[column]):
                changed.append(column)
...
def run_apply(args: argparse.Namespace) -> None:
    ...
    flypoint_event_rows: List[Dict[str, object]] = []
    total_valve_negative_corrections = 0
    for file_index, path in enumerate(files, start=1):
        ...
        valve_correction_count, valve_corrections_by_signal = clamp_negative_valves(after)
        total_valve_negative_corrections += valve_correction_count
        ...
        manifest_rows.append(
            {
                ...
                "valve_negative_values_clamped": valve_correction_count,
                "valve_negative_corrections_by_signal": json.dumps(
                    valve_corrections_by_signal, ensure_ascii=False, sort_keys=True
                ),
                **file_flypoint_stats,
                "protected_columns_exact": protected_ok_disk and valve_correction_count == 0,
                "protected_columns_contract_ok": protected_ok_disk,
    ...
    config = {
        ...
        "valve_filtering": False,
        "valve_smoothing": False,
        "valve_negative_clamp": True,
        "valve_minimum": 0.0,
        "valve_negative_values_clamped": total_valve_negative_corrections,
        "valve_zero_is_valid": True,
        "data_split": False,
        "label_creation": False,
    }
