...
DEFAULT_EXPECTED_LOOKBACKS = tuple(range(10, 61, 5))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--expected-lookbacks",
        default=",".join(str(value) for value in DEFAULT_EXPECTED_LOOKBACKS),
        help=(
            "Comma-separated lookbacks expected in the formal comparison. "
            "Defaults to 10,15,...,60."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Required common history origin recorded in each run. Defaults "
            "to the largest value in --expected-lookbacks."
        ),
    )
...
def parse_lookback_set(text: str) -> tuple[int, ...]:
    lookbacks = tuple(
        sorted({int(item.strip()) for item in text.split(",") if item.strip()})
    )
    if not lookbacks:
        raise ValueError("--expected-lookbacks must contain at least one integer")
    if any(value <= 0 for value in lookbacks):
        raise ValueError("--expected-lookbacks values must all be positive")
    return lookbacks
...
def load_runs(
    ...
    for path in paths:
        ...
        if int(payload.get("common_origin_lookback", -1)) != common_origin_lookback:
            raise ValueError(
                f"Result does not use common {common_origin_lookback}-step "
                f"origins: {path}"
            )
...
def main() -> None:
    ...
    expected_lookbacks = parse_lookback_set(args.expected_lookbacks)
    common_origin_lookback = (
        max(expected_lookbacks)
        if args.common_origin_lookback is None
        else args.common_origin_lookback
    )
    if common_origin_lookback < max(expected_lookbacks):
        raise ValueError(
            "--common-origin-lookback cannot be smaller than the largest "
            "expected lookback"
        )
