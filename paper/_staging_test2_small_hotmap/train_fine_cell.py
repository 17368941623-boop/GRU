#!/usr/bin/env python3
"""Train one leakage-safe cell of the local GRU lookback/hidden grid.
...
def read_contract(data_dir: Path) -> tuple[dict, tuple[str, ...], tuple[str, ...]]:
    ...
    if PREDICT_STEPS not in set(map(int, build.get("horizons", []))):
        raise ValueError("The processed dataset has no 15-step target")
...
def main() -> None:
    ...
    predictions = pd.DataFrame(
        {
            ...
            "actual_delta_Thv_15step_k": actual_delta,
            "predicted_delta_Thv_15step_k": predicted_delta,
            "actual_Future_Thv_15step_k": actual,
            "predicted_Future_Thv_15step_k": predicted,
            "residual_k": predicted - actual,
            "rapid_cooling_mask": rapid_mask.astype(np.int8),
        }
    )
    target_timestamps = (
        pd.to_datetime(predictions["source_timestamp"], errors="raise")
        + pd.to_timedelta(PREDICT_STEPS * SAMPLE_INTERVAL_SECONDS, unit="s")
    )
    predictions.insert(
        predictions.columns.get_loc("source_timestamp") + 1,
        "target_timestamp",
        target_timestamps.dt.strftime("%Y-%m-%d %H:%M:%S"),
    )
    predictions["difficult_window_id"] = ""
    difficult_union = np.zeros(len(predictions), dtype=bool)
    difficult_window_metrics: dict[str, dict[str, float | int | str]] = {}
    for window_id, start_text, end_text in DIFFICULT_WINDOWS:
        start = pd.Timestamp(start_text)
        end = pd.Timestamp(end_text)
        mask = ((target_timestamps >= start) & (target_timestamps <= end)).to_numpy()
        count = int(mask.sum())
        ...
        if np.any(difficult_union & mask):
            raise ValueError(f"Difficult windows overlap at {window_id}")
        difficult_union |= mask
        predictions.loc[mask, "difficult_window_id"] = window_id
        window_metrics = regression_metrics(actual[mask], predicted[mask])
        window_persistence = regression_metrics(actual[mask], current[mask])
        difficult_window_metrics[window_id] = {
            "target_start": start_text,
            "target_end": end_text,
            "targets": count,
            **window_metrics,
            "persistence_rmse_k": float(window_persistence["rmse_k"]),
        }
    difficult_count = int(difficult_union.sum())
    if difficult_count != EXPECTED_DIFFICULT_TARGETS:
        raise ValueError(
            f"Difficult-window union contains {difficult_count} targets; "
            f"expected {EXPECTED_DIFFICULT_TARGETS}"
        )
    difficult_metrics = regression_metrics(
        actual[difficult_union], predicted[difficult_union]
    )
    difficult_persistence_metrics = regression_metrics(
        actual[difficult_union], current[difficult_union]
    )
    ...
    payload = {
        ...
        "predict_steps": PREDICT_STEPS,
        "seed": args.seed,
        "target": "Delta_Thv_15step",
        "source_label": LABEL_COLUMN,
        "history_feature_columns": list(history_columns),
        "future_control_columns": list(control_columns),
        "future_control_alignment": "u(t)..u(t+14)",
        ...
        "validation_metrics": validation_metrics,
        "difficult_validation_targets": difficult_count,
        "difficult_validation_metrics": difficult_metrics,
        "difficult_window_metrics": difficult_window_metrics,
        "difficult_persistence_metrics": difficult_persistence_metrics,
    ...
    print(
        f"HARD_COOLING_POOLED_RMSE_K={difficult_metrics['rmse_k']:.10f}",
        flush=True,
    )
