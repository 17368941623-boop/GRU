...
SAVE_DIR = ROOT / "saved_models_torch_90"
...
ignore_keywords = ["Fluid_Density", "Fluid_Enthalpy"]
feature_cols = [
...
#print("核心新增特征:", [c for c in ["CV8310_diff", "CV8310_ratio", "TE8353_slope_60", "TE8353_TE8310_diff"] if c in feature_cols])

scaler_x = StandardScaler()
scaler_y = StandardScaler()
train_x_s = scaler_x.fit_transform(df_train[feature_cols]).astype(np.float32)
train_y_s = scaler_y.fit_transform(df_train[[DELTA_LABEL]]).astype(np.float32)
...
def create_sequences(df_ref, x_data, y_delta_scaled):
    x_seq, x_mask, x_aux, y_delta, y_curr_abs, y_fut_abs, sample_groups = [], [], [], [], [], [], []
    ...
    # aux 通道: 伪冷量公式所需的物性量 (Fluid_* 列被排除出特征, 单独走 aux 供反事实扰动重算)
    # a0 = sqrt(ρ_8310), a1 = sqrt(ρ_8310)*H_8310  ->  Pseudo_Mass_Flow = (CV8300+CV8313)*a0
    if "Fluid_Density_8310" in df_ref.columns and "Fluid_Enthalpy_8310" in df_ref.columns:
        ...
        a0 = np.sqrt(np.clip(rho, 0.0, None))
    ...
    for fid in df_ref["file_id"].unique():
        ...
        for i in range(len(x_sub) - LOOK_BACK):
...
print("正在构建时序样本...")
X_train_full, M_train_full, AUX_train_full, y_delta_full, y_curr_full, y_fut_full, group_full = create_sequences(df_train, train_x_s, train_y_s)
X_test, M_test, _, y_delta_test, y_curr_test, y_fut_test, _ = create_sequences(df_test, test_x_s, test_y_s)
...
val_dataset = TensorDataset(torch.from_numpy(X_v), torch.from_numpy(M_v), torch.from_numpy(AUX_v), torch.from_numpy(y_v))
...
def _find_feature_col(key):
    key_up = key.upper()
    for c in feature_cols:
        if str(c).upper() == f"{key_up} VALUEY":
            return c
    for c in feature_cols:
        cu = str(c).upper()
...
_pert_names = {
    "CV8300": _find_feature_col("CV8300"),
    "CV8313": _find_feature_col("CV8313"),
    "CV8310": _find_feature_col("CV8310"),
    "CV8311": _find_feature_col("CV8311"),
    "CV8312": _find_feature_col("CV8312"),
    "CV8351": _find_feature_col("CV8351"),
}
for _derived in ["CV8310_ratio", "Pseudo_Mass_Flow", "Pseudo_Cooling_Power", "Pseudo_Mass_Flow_8310", "Pseudo_Cooling_Power_8310"]:
    _pert_names[_derived] = _derived if _derived in feature_cols else None
PERT_IDX = {k: feature_cols.index(v) for k, v in _pert_names.items() if v is not None}
missing = [k for k, v in _pert_names.items() if v is None]
if missing:
...
print("开始训练 PyTorch GRU ΔT 模型 (含单调性物理约束)...")
n_blocks = 1 + len(MONO_VALVES)
for epoch in range(1, EPOCHS + 1):
    model.train()
    train_losses, train_monos = [], []
    for xb, mb, auxb, yb, wb in train_loader:
        ...
        loss = sup_loss + LAMBDA_MONO * mono_loss
        loss.backward()
    ...
    val_metric = val_loss + LAMBDA_MONO * val_mono
    ...
    print(
        f"Epoch {epoch:03d}/{EPOCHS} - train: {train_loss:.6f} (mono {train_mono:.6f}) "
        f"- val: {val_loss:.6f} (mono {val_mono:.6f}) - 违反率/均幅 {viol_str}"
    )
...
print("Training finished. Showing result figure now. Use the toolbar to zoom; close the window to save PNG and model.")
