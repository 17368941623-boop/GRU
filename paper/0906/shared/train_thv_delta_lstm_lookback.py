...
def train_model(
    ...
    selection_metric = getattr(
        args, "selection_metric", "validation_rapid_rmse_k"
    )
    if selection_metric == "validation_rmse_k":
        monitor_key = "rmse_k"
    elif selection_metric == "validation_rapid_rmse_k":
        monitor_key = "rapid_rmse_k"
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step(val_stats[monitor_key])
        ...
        monitored = val_stats[monitor_key]
        if monitored < best_monitored_rmse - args.min_delta:
            best_monitored_rmse = monitored
            best_epoch = epoch
            best_state = {
            ...
            if epochs_without_improvement >= args.patience:
                print(
                    ...
                    f"best_{selection_metric}={best_monitored_rmse:.8f}"
                )
                break

    if best_state is None:
        raise RuntimeError("Training did not produce a checkpoint")
    return best_state, history_rows, best_epoch, best_monitored_rmse
