#!/usr/bin/env python3
"""Train one cell of the 0910 architecture-by-cumulative-feature matrix.
...
No code in this program opens a test pickle.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train one leakage-safe model-feature heatmap cell."
    )
...
def main() -> None:
    args = parse_args()
    args.config_id = f"{args.model}__{args.feature_set}"
    args.lookback = LOOKBACK
    args.common_origin_lookback = COMMON_ORIGIN_LOOKBACK
    args.predict_steps = PREDICT_STEPS
    validate_args(args)
    architecture = model_spec(args.model)
    spec = feature_spec(args.feature_set)
    history_feature_cols = history_columns(spec)
    active_feature_cols = active_history_columns(spec)
    history_mask = np.asarray(history_feature_mask(spec), dtype=np.float32)
    if not set(history_feature_cols).issuperset(comparison_helpers.HISTORY_FEATURE_COLS):
        raise AssertionError("Every ablation must preserve all 20 raw history inputs")
    # The imported leakage/boundary helpers resolve this module constant at
    # call time. Each process trains exactly one feature set, so this update is
    # local to the process and makes every audit use the selected columns.
    comparison_helpers.HISTORY_FEATURE_COLS = history_feature_cols
    set_seed(args.seed)
    ...
    run_dir = run_directory(args.results_dir, architecture, spec, args.seed)
    ...
    train_history = history_scaler.transform(
        train_frame[list(history_feature_cols)].to_numpy(dtype=np.float64)
    ).astype(np.float32) * history_mask
    ...
    active_set = set(active_feature_cols)
    normalization_report["active_model_input"] = [
        (branch != "history") or (feature in active_set)
        for branch, feature in zip(
            normalization_report["branch"], normalization_report["feature"]
        )
    ]
    ...
    print(
        f"FEATURE_SET={args.feature_set} | ACTIVE_HISTORY={len(active_feature_cols)} | "
        f"MASKED_ENGINEERED={int(np.sum(history_mask == 0.0))}"
    )
    ...
    metadata = {
        "experiment": "Thv_h15_lb60_architecture_cumulative_feature_matrix",
        "model_type": args.model,
        "model_description": model_description(args.model),
        "config_id": args.config_id,
        "feature_set": args.feature_set,
        "feature_set_label": spec["label"],
        "feature_set_role": spec["role"],
        "feature_stage_index": int(spec["stage_index"]),
        "introduced_feature_group": spec.get("introduced_group"),
        "engineered_feature_columns": list(spec["engineered"]),
        "active_history_feature_columns": list(active_feature_cols),
        "masked_history_feature_columns": [
            column for column, keep in zip(history_feature_cols, history_mask)
            if not keep
        ],
        "cumulative_masking_policy": (
            "all configurations retain the same 42 input channels and parameter "
            "count; inactive engineered channels are zero after train-fitted "
            "standardization"
        ),
