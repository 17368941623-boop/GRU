#!/usr/bin/env python3
"""Small statistical helpers shared by the 0910 summaries."""
...
def paired_row(
    ...
    if expected_seeds is not None and len(matched) != expected_seeds:
        raise ValueError(
            f"{comparison_id} has {len(matched)} matched seeds; expected {expected_seeds}"
        )
    differences = (
...
def paired_table(
    ...
    table = pd.DataFrame([
        paired_row(runs, left, right, metric, comparison_id, expected_seeds)
