...
SAVE_DIR = ROOT / "saved_models_gru_ode"
...
def resolve_feature_col(key):
    if key in feature_cols:
        return key
    return find_feature_col(key)


CONTROL_COLS = []
missing_controls = []
for key in CONTROL_KEYS:
    col = resolve_feature_col(key)
    if col is None:
        missing_controls.append(key)
    else:
        CONTROL_COLS.append(col)
if missing_controls:
    print(f"[warn] ODE 控制量缺失, 已跳过: {missing_controls}")
if not CONTROL_COLS:
    raise ValueError("ODE 控制量为空，无法构建 GRU-ODE 模型")
CONTROL_IDX = [feature_cols.index(c) for c in CONTROL_COLS]
print(f"ODE 控制输入列({len(CONTROL_COLS)}): {CONTROL_COLS}")
...
class ODEFunc(nn.Module):
    def __init__(self, z_dim, u_dim, hidden_dim=64):
        super().__init__()
...
def rk4_integrate(func, z0, u, steps=10, horizon=1.0):
    z = z0
    h = horizon / steps
    for _ in range(steps):
        k1 = func(z, u)
        k2 = func(z + 0.5 * h * k1, u)
        k3 = func(z + 0.5 * h * k2, u)
        k4 = func(z + h * k3, u)
...
class GRUTemperatureModel(nn.Module):
    def __init__(self, input_size, control_idx, hidden_size=128, time_steps=60, dropout=0.2, z_dim=32, ode_hidden=64, ode_steps=10, ode_horizon=1.0):
        super().__init__()
        ...
        self.ode_func = ODEFunc(z_dim=z_dim, u_dim=len(control_idx), hidden_dim=ode_hidden)
        self.head = nn.Sequential(nn.Linear(z_dim, 32), nn.ReLU(), nn.Linear(32, 1))
        self.ode_steps = ode_steps
        self.ode_horizon = ode_horizon
        self.register_buffer("control_idx", torch.tensor(control_idx, dtype=torch.long))
    ...
    def forward(self, x, mask):
        ...
        u = x[:, -1, self.control_idx]
        zH = rk4_integrate(self.ode_func, z0, u, steps=self.ode_steps, horizon=self.ode_horizon)
        return self.head(zH), alpha
...
print("开始训练 PyTorch GRU-ODE ΔT 模型 (含单调性物理约束 + TE8351 下界约束)...")
...
ax1.set_title(f"PyTorch Delta-GRU-ODE Accuracy ({PREDICT_STEPS} Steps Ahead)\nMAE: {mae:.2f} K | R²: {r2:.4f}")
...
plt.tight_layout()
plot_path = ROOT / f"torch_gru_ode_delta_prediction_accuracy_ahead{PREDICT_STEPS}.png"
plt.savefig(plot_path, dpi=200)
print(f"预测图已保存: {plot_path}")

model_path = SAVE_DIR / "gru_ode_temperature_model_torch.pt"
torch.save(
    {
        "model_state_dict": model.state_dict(),
        "model_type": "prior_guided_gru_ode",
